<template>
  <div class="space-y-6">
    <!-- Tabs -->
    <div class="border-b border-gray-200">
      <nav class="flex space-x-6">
        <button
          v-for="tab in tabs" :key="tab"
          @click="activeTab = tab"
          class="pb-3 text-sm font-medium transition border-b-2"
          :class="activeTab === tab
            ? 'border-blue-600 text-blue-600'
            : 'border-transparent text-gray-500 hover:text-gray-700'"
        >
          {{ tab }}
        </button>
      </nav>
    </div>

    <!-- Purchase Orders -->
    <div v-if="activeTab === 'Orders'">
      <div class="flex justify-between items-center mb-4">
        <h3 class="text-sm font-semibold text-gray-700">Purchase Orders</h3>
        <button class="bg-blue-800 text-white text-sm px-4 py-2 rounded-lg hover:bg-blue-900">
          + New Order
        </button>
      </div>
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 text-gray-400 text-xs uppercase">
            <tr>
              <th class="text-left px-4 py-3">Vendor</th>
              <th class="text-left px-4 py-3">Order Date</th>
              <th class="text-right px-4 py-3">Total</th>
              <th class="text-left px-4 py-3">Status</th>
              <th class="text-left px-4 py-3">Actions</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="orders.length === 0">
              <td colspan="5" class="px-4 py-6 text-center text-gray-400">No purchase orders found</td>
            </tr>
            <tr v-for="o in orders" :key="o.id"
              class="border-t border-gray-50 hover:bg-gray-50">
              <td class="px-4 py-3 text-gray-800">{{ o.vendor }}</td>
              <td class="px-4 py-3 text-gray-500">{{ o.order_date }}</td>
              <td class="px-4 py-3 text-right text-gray-800 font-medium">{{ o.total }}</td>
              <td class="px-4 py-3">
                <span :class="statusClass(o.status)"
                  class="px-2 py-1 rounded-full text-xs font-medium capitalize">
                  {{ o.status }}
                </span>
              </td>
              <td class="px-4 py-3 space-x-2">
                <button v-if="o.status === 'draft'"
                  @click="submitOrder(o.id)"
                  class="text-xs text-blue-600 hover:underline">
                  Submit
                </button>
                <button v-if="o.status === 'pending'"
                  @click="approveOrder(o.id)"
                  class="text-xs text-green-600 hover:underline">
                  Approve
                </button>
              </td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>

    <!-- Vendors -->
    <div v-if="activeTab === 'Vendors'">
      <div class="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden">
        <table class="w-full text-sm">
          <thead class="bg-gray-50 text-gray-400 text-xs uppercase">
            <tr>
              <th class="text-left px-4 py-3">Name</th>
              <th class="text-left px-4 py-3">Email</th>
              <th class="text-left px-4 py-3">Currency</th>
              <th class="text-right px-4 py-3">Payment Terms</th>
            </tr>
          </thead>
          <tbody>
            <tr v-if="vendors.length === 0">
              <td colspan="4" class="px-4 py-6 text-center text-gray-400">No vendors found</td>
            </tr>
            <tr v-for="v in vendors" :key="v.id"
              class="border-t border-gray-50 hover:bg-gray-50">
              <td class="px-4 py-3 text-gray-800">{{ v.name }}</td>
              <td class="px-4 py-3 text-gray-500">{{ v.email }}</td>
              <td class="px-4 py-3 text-gray-500">{{ v.currency }}</td>
              <td class="px-4 py-3 text-right text-gray-600">{{ v.payment_terms }} days</td>
            </tr>
          </tbody>
        </table>
      </div>
    </div>
  </div>
</template>

<script setup>
import { ref, onMounted } from 'vue'
import client from '../api/client'

const activeTab = ref('Orders')
const tabs      = ['Orders', 'Vendors']
const orders    = ref([])
const vendors   = ref([])

function statusClass(status) {
  const map = {
    draft:     'bg-gray-100 text-gray-600',
    pending:   'bg-yellow-100 text-yellow-600',
    approved:  'bg-green-100 text-green-600',
    received:  'bg-blue-100 text-blue-600',
    cancelled: 'bg-red-100 text-red-600',
  }
  return map[status] || 'bg-gray-100 text-gray-600'
}

async function submitOrder(id) {
  try {
    await client.post(`/purchases/orders/${id}/submit/`)
    const res = await client.get('/purchases/orders/')
    orders.value = res.data.results || res.data
  } catch {}
}

async function approveOrder(id) {
  try {
    await client.post(`/purchases/orders/${id}/approve/`)
    const res = await client.get('/purchases/orders/')
    orders.value = res.data.results || res.data
  } catch {}
}

onMounted(async () => {
  try {
    const res = await client.get('/purchases/orders/')
    orders.value = res.data.results || res.data
  } catch {}

  try {
    const res = await client.get('/finance/vendors/')
    vendors.value = res.data.results || res.data
  } catch {}
})
</script>